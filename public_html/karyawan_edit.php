...
// 3. AMBIL DATA LAMA
$sql_item = "SELECT KaryawanID, NamaKaryawan, Username, Role FROM KARYAWAN WHERE KaryawanID = ?";
$stmt_item = $koneksi->prepare($sql_item);
$stmt_item->bind_param("i", $karyawan_id_edit);
$stmt_item->execute();
...
<head>
    ...
    <style>
        ...
        .password-note { font-size: 0.9rem; color: #555; }
    </style>
</head>
<body>
    ...
    <div class="dashboard-container">
        <nav class="sidebar">
             ...
             <ul class="sidebar-menu">
                ...
                <li class="active-sub"><a href="karyawan.php">Manajemen Karyawan</a></li>
             </ul>
        </nav>
        ...
        <main class="main-content">
            ...
            <div class="content-wrapper">
                <div class="form-card">
                    <h3>Edit Detail: <?php echo htmlspecialchars($item['NamaKaryawan']); ?></h3>
                    <form action="proses_karyawan.php" method="POST">
                        <input type="hidden" name="action" value="update"> 
                        <input type="hidden" name="karyawan_id" value="<?php echo htmlspecialchars($item['KaryawanID']); ?>"> 
                        
                        <div class="form-group">
                            <label for="nama_karyawan">Nama Karyawan (Wajib)</label>
                            <input type="text" id="nama_karyawan" name="nama_karyawan" value="<?php echo htmlspecialchars($item['NamaKaryawan']); ?>" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="username">Username (Wajib, Unique)</label>
                            <input type="text" id="username" name="username" value="<?php echo htmlspecialchars($item['Username']); ?>" required>
                        </div>

                        <div class="form-group">
                            <label for="role">Role (Hak Akses)</label>
                            <select id="role" name="role" required>
                                <option value="karyawan" <?php if ($item['Role'] == 'karyawan') echo 'selected'; ?>>Karyawan</option>
                                <option value="kasir" <?php if ($item['Role'] == 'kasir') echo 'selected'; ?>>Kasir</option>
                                <option value="admin" <?php if ($item['Role'] == 'admin') echo 'selected'; ?>>Admin</option>
                                <option value="pemilik" <?php if ($item['Role'] == 'pemilik') echo 'selected'; ?>>Pemilik</option>
                            </select>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <p class="password-note">Hanya isi bidang ini jika Anda ingin mereset password karyawan.</p>
                        </div>
